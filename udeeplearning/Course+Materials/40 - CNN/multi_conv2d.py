...
model = nn.Sequential(
    nn.Sequential(
        nn.Conv2d(1, 3, kernel_size=(3, 3), padding=1, padding_mode="reflect"), nn.MaxPool2d(kernel_size=2), nn.ReLU()
    ),
    nn.Sequential(
    ...
    nn.Flatten(),
    nn.Sequential(nn.Linear(294, 100), nn.ReLU(), nn.Linear(100, 10)),
).to(device)
print(model)
